...
#sample_names <- c(1:length(files))
pos <- regexpr("CRC\\d+", files, perl=TRUE)
#for (i in 1:length(pos)) {  sample_names[i] <- substr(files[i],pos[i],pos[i]+6) }
sample_names <- mapply(function(x, p) {substr(x,p,p+6)}, files, pos)
vcfs <- read_vcfs_as_granges(files, sample_names, ref_genome)

setwd(outputdir)
base_clone <- sample_names
