...
# What is the depth?
depth     = extract.gt(vcf, element = "NR", as.numeric = TRUE, mask=!is.indel(vcf)) #return.indels=FALSE)
# How many reads support the variant?
variant   = extract.gt(vcf, element = "NV", as.numeric = TRUE, mask=!is.indel(vcf))
# Calculate the vaf
vafs = variant / depth
...
# Calculate the vaf
bvafs = extract.gt(bvcf, element = "AF", as.numeric = TRUE, mask=!is.indel(vcf))
# Use threshold for vaf
bvafs = bvafs >= vt
# Convert to binary table
