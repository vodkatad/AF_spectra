...
LEVEL <- 0.99
ic_clones <- sapply(unique(fit_r2$mp), function(x) { confidence_interval(fit_r2[fit_r2$mp==x, column], LEVEL) })
...
p <- ggplot(data=fit_r2, aes(x=model, y=intercept, color=mp, group=smodel)) +
  ...
  ylab(ylab)+xlab('PDXs')+unmute_theme+theme(legend.position="none")+
