...
ggsave(outplot, plot=pp, width=89, height=89, units="mm")
...
###
exit(0)
load('/scratch/trcanmed/AF_spectra/dataset_Figures_Tables/fig_6a_eo.Rdata')
...
pp <- ggplot() + 
  geom_point(data=our, aes(x=model, y=MR, color=type, group=sample), position=position_dodge(0.8), size=3, shape=18)+
  geom_segment(data=pdata, aes(x=xmodel-0.2, yend=mean,y=mean,  xend=xmodel+0.2),size=.3) +
  geom_errorbar(data=pdata, aes(ymin=lower, ymax=upper, x=model), size=0.3, width=0.3)+ylab('MR [SNV/(Gbp*division)]')+xlab('')+
  scale_color_manual(values=c('#adacac', '#595959'))+
  scale_y_continuous(breaks=y_breaks,limits=c(0, max(y_breaks)),expand = c(0, 0))+# + ylim(min(y_breaks),max(y_breaks))+
...
ggsave('/scratch/trcanmed/AF_spectra/temp/MR_leg.svg', plot=pp, width=89, height=89, units="mm")
