...
colnames(ic_clones) <- unique(our$model_well)
pdata <- as.data.frame(t(ic_clones))
pdata$model_well <- rownames(pdata)
pdata$model <- sapply(pdata$model_well, function(x) {y<-strsplit(x, '_')[[1]][1]; return(y[1])})
...
p <- ggplot(pdata, aes(x=ord_x_time, y=mean))+  #geom_point(stat="identity", shape=1, size=2) 
  geom_point(data=our, aes(x= ord_x_time, y=EDU, color=model_clone), stat="identity", size=2, shape=18,position=position_dodge(0.3))+#, 
  geom_segment(data=pdata, aes(x= ord_x_time-0.2, yend=mean,y=mean,  xend= ord_x_time+0.2),size=.3)+
  geom_errorbar(aes(ymin=lower, ymax=upper, x= ord_x_time), size=0.3)+ylab('% cells EDU+')+xlab('PDTs')+#xmin=reorder(model_time, ord_x_time), xmax=reorder(model_time, ord_x_time)
  scale_color_manual(values=pal)+unmute_theme+
  scale_y_continuous(breaks=y_breaks,limits=c(0, 50),expand = c(0, 0)) +#, expand = c(0, 0))+
