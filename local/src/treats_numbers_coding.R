...
all <- read.table('/scratch/trcanmed/AF_spectra/dataset_MAtreats/all_muts.tsv', sep="\t", header=F, stringsAsFactors = F)
colnames(all) <- c('n', 'model')
coding <- read.table('/scratch/trcanmed/AF_spectra/dataset_MAtreats/all_muts_coding.tsv', sep="\t", header=T, stringsAsFactors = F)
...
setwd('/scratch/trcanmed/AF_spectra/dataset_MAtreats')
ggsave('exon.svg', plot=p, width=89, height=89, units="mm")
...
p <- ggplot(as.data.frame(longfp), aes(x = reorder(tmodel, order), y = perc, fill = Var1)) +
  ...
  geom_text(aes(label = Freq),
            position = position_stack(vjust = 0.5)) +
  scale_fill_manual(values = c("aquamarine", "steelblue", 'slategray4'))+
  unmute_theme + guides(col=guide_legend(nrow=length(pal), keyheight=unit(0.01, "mm")))+
  scale_y_continuous(breaks=y_breaks, limits=c(0,1), expand = c(0, 0))+
  scale_x_discrete(labels=c('NT_CRC1430'="NT", 'T_CRC1430'="Afatinib", 'NT_CRC1620'="NT", 'T_CRC1620'="Afatinib"))+
  xlab('')+ylab('% accumulated SNVs')

ggsave('coding.svg', plot=p, width=89, height=89, units="mm")
