...
baseline <- read.table(baseline, header=TRUE, sep="\t")
...
gained <- df[df$Var1=="gain","Freq"]

name_num <- strsplit(len,"_")[[1]][1]
name_den <- strsplit(len,"_")[[1]][2]
name_den <- strsplit(name_den,'.', fixed=TRUE)[[1]][1]
clone <- strsplit(name_num, '-')[[1]][2]
colnames(generation) <- c("CAMPIONI", "GENS", "TIPO","GIORNI")
generation <- generation[generation$CAMPIONI==clone,]

## here we need to subtract baseline
base_gained <- baseline[baseline$clone == as.numeric(clone), 'n_gained']
gained <- gained - base_gained
...
res <- data.frame(clone=clone, end=name_num, start=name_den, class=type, n_gained=gained, len_cnok=glen, MR_conte=MR[1], MR_EDU=MR[2], MR_giorni=MR_time)

write.table(res, args[5], sep="\t", col.names=TRUE, row.names=FALSE, quote=FALSE)
