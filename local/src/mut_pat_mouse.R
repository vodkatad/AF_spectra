...
ggplot(cos_sim_ori_rec, aes(y=cos_sim, x=reorder(s, n), fill=sample)) +
  ...
  xlab("") +theme_bw(base_size=20)+ theme(axis.text.y = element_text(size=10), axis.title.x=element_text(size=15))+
  geom_hline(aes(yintercept=.95))


dd <- as.data.frame(colSums(mut_mat))
m <- merge(dd, cos_sim_ori_rec, by="row.names")
colnames(m)[2] <- 'N_SNP'
ggplot(data=m, aes(x=N_SNP, y=cos_sim))+geom_point()+theme_bw(base_size = 20)
