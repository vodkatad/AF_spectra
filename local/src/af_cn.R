...
sample_name<-strsplit(mutfile,split='-')[[1]][1]
...
p <- p + scale_y_continuous(breaks=y_breaks,limits=c(0, max(y_breaks)),expand = c(0, 0),labels=y_labels)+# + ylim(min(y_breaks),max(y_breaks))+
  scale_x_continuous(breaks=c(0,0.25,0.5,0.75,1),limits=c(0, 1),expand = c(0, 0),labels=c('0','0.25','0.5','0.75','1'))+facet_grid(. ~ cn)+# + ylim(min(y_breaks),max(y_breaks))+ #
...
save.image('p.Rdata')
ggsave(file=outputplot, plot=p,width = 180,height=40,units = "mm",dpi = 300)
